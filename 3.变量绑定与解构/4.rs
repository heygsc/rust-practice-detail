...
//解法一
//首先，println用到了x，需要将x初始化。这道题既然定义了函数，我们就将它初始化成define_x()。
//define_x函数这边，->的意义，是确定返回值的类型，这里我们返回String（字符串）。
//接下来就是函数体。
//在函数体当中，第一行使用to_string，将它转为String。
//为什么要使用to_string呢？因为原来的类型是&str，函数需要的返回值为String。我们需要使用to_string将&str转为String。
