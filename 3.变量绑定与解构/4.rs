...
//为什么要使用 to_string 呢？因为原来的类型是 &str ，函数需要的返回值为 String 。
...
//:?属于格式化输出，所以最终的输出中，hello 外面会出现双引号。
//函数的定义这边， &'static str 是返回了 &str，所以不用 to_string 了。而 'static ，则是生命周期相关的。
